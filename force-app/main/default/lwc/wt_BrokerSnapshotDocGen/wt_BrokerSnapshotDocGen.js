...
import generate from '@salesforce/apex/WT_BrokerSnapshotDocGenController.generate';
...
export default class Wt_BrokerSnapshotDocGen extends NavigationMixin(LightningElement) {
    ...
    get generateButtonLabel() {
        return this.isGenerating ? 'Generating…' : 'Generate Broker Snapshot PDF';
    }
    ...
    async handleGenerateClick() {
        ...
        try {
            ...
            this.applyDocumentResult(result, { openPreview: true });
            this.showToast('Broker Snapshot generated', `${result.fileName} is ready.`, 'success');
        } catch (error) {
    ...
    handleDownloadClick() {
        ...
        anchor.download = this.downloadFileName || 'Broker Snapshot.pdf';
    ...
    fail(message) {
        ...
        this.showToast('Could not generate Broker Snapshot', message, 'error');
    }
